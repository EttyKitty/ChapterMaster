function scr_arti_descr() {
    ...
    if (custom_description == "") {
        ...
        if (!inq_mission && has_tag("chaos_gift")) {
            mission_data = $"This artifact is a {type()} gifted by the Chaos Lord.";
        }
    ...
    if (has_tag("MINOR")) {
        other_data += "It is more crude and utilitarian than one might expect from an artifact.";
    }
    if (has_tag("chaos")) {
        p4 = "It bears the taint of Chaos.";
    }
    if (has_tag("daemonic")) {
        p4 = "It is infested with a Daemonic entity. Destroying it, may cause the entity to materialize.";
    }

    final_description += mission_data;
    if (basic_asthetic != "") {
        final_description += $"  {basic_asthetic}";
    }
    if (other_data != "") {
        final_description += $"  {other_data}";
    }
    if (p4 != "") {
        final_description += $"  {p4}";
    }
