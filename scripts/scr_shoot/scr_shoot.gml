...
function combat_rank_armour(_ac, _ap, _is_vehicle) {
    ...
    _armor_mod = _is_vehicle ? _veh[_ap] : _inf[_ap];
...
function combat_apply_rank_damage(block, rank, dmg_per_weapon, ap, splash, shots_fired) {
    ...
    var _final_hit = max(0, (dmg_per_weapon - (_rank_armour * splash)) * _rank_dr);
    ...
    _casualties = min(_casualties, shots_fired * splash);
...
function scr_shoot_enemy(weapon_index_position, target_object, damage_data, melee_or_ranged, aggregate_damage, armour_pierce) {
    ...
    if (_damage_type == "status") {
        if (melee_or_ranged != "wall") {
            ...
            scr_clean(target_object, _hostile_type, _hit_count, _damage, _weapon_name, _weapon_range, _weapon_splash, weapon_index_position);
        }
...
function scr_shoot_player(weapon_index_position, target_object, target_type, aggregate_damage, armour_pierce) {
    ...
    var _invalid_target = false;

    // Skip past dead ranks
    ...
    // Handle weapon resources (ammo and silos)
    var _has_ammo = true;
    if (weapon_index_position >= 0) {
    ...
    var _can_proceed = !_invalid_target && _has_ammo;
    if (_can_proceed) {
        if (weapon_index_position >= 0) {
            _can_proceed = aggregate_damage > 0;
        } else if (weapon_index_position < -40) {
            _can_proceed = true;
        }
    }
    ...
    // Set up weapon damage parameters
