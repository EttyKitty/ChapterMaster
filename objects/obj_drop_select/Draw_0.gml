...
if (attack=0) then draw_text_transformed(xx+550,yy+212,string_hash_to_newline("Raiding ("+string(p_target.name)+" "+scr_roman(obj_controller.selecting_planet)+")"),0.6,0.6,0);
if (attack=1) then draw_text_transformed(xx+550,yy+212,string_hash_to_newline("Attacking ("+string(p_target.name)+" "+scr_roman(obj_controller.selecting_planet)+")"),0.6,0.6,0);
...
// Formation
formation.pos_x = xx + 550;
formation.pos_y = yy + 245;
...
repeat(50){                                 // Ship Forces here
    if (ship[e]!="") and (ship_max[e]>0){
        ...
        y8+=18;sip+=1;
        if (y8>=yy+299+108){y8=yy+299;x8+=168;}
    }
    e+=1;
}
...
draw_text_ext(xx+550,yy+438,string_hash_to_newline(string(sel)),-1,590);

draw_text(xx + 550,yy + 527,"Selected Squads:");
for (var i = 0; i < 8; i++){
    squad_buttons[i].w = 80;
    squad_buttons[i].pos_x = xx + 553 + round((i % 5) * 96);
    squad_buttons[i].pos_y = yy + 555 + floor(i / 5) * 32;
    if (i = 7) squad_buttons[i].pos_x += 90;
...
target.draw();
draw_sprite(spr_faction_icons, attacking,xx+1084,yy+245);
var q = 0;
repeat(20) {
    q += 1;
...
// Back / Purge buttons
btn_back.pos_x = xx+994;
btn_back.pos_y = yy+594;
btn_back.str1 = "BACK";
btn_back.draw();
if (btn_back.clicked() and (obj_controller.cooldown<=0)){
    obj_controller.cooldown=8000;menu=0;purge=0;instance_destroy();
}

// Attack / Raid buttons
btn_attack.pos_x = xx+1054;
btn_attack.pos_y = yy+594;
