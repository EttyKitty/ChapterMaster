...
function ToggleButton() constructor {
    ...
    hover = function() {
        var str1_h = string_height(str1);
        return (mouse_x >= pos_x-2 && mouse_x <= pos_x+w+1 && mouse_y >= pos_y-4 && mouse_y <= pos_y+str1_h+1);
    };
    clicked = function() {
    ...
    draw = function() {
        var str1_h = string_height(str1);
        if (!active){
            if (state_alpha > 0.5) state_alpha -= 0.02;
        }
        else{
            if (state_alpha < 1) state_alpha += 0.03;
            if (hover()) {
        ...
        draw_set_alpha(state_alpha * hover_alpha); // Multiply state_alpha and click_alpha to get the final state_alpha value
        draw_set_color(c_green);
        draw_rectangle(pos_x-2, pos_y-4, pos_x+w+1, pos_y+str1_h+1,1);
        draw_set_halign(fa_left);
        draw_set_valign(fa_top);
        draw_text(pos_x, pos_y,str1);
        draw_set_alpha(1);
